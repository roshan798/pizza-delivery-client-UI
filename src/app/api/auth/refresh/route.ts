...
export async function POST() {
    ...
    if (!res.ok) {
        return new Response(JSON.stringify({ message: 'Failed to refresh access token' }), {
            status: 401,
            headers: {
                'Content-Type': 'application/json',
            },
    ...
    return new Response(JSON.stringify({ message: 'Access token refreshed, but no cookies set' }), {
        status: 200,
        headers: {
            'Content-Type': 'application/json',
        },
